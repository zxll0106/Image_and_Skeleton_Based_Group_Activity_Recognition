...
def train_net(cfg):
    ...
    # Training iteration
    best_result={'epoch':0, 'activities_MPCA':0}
    start_epoch=1
    for epoch in range(start_epoch, start_epoch+cfg.max_epoch):
        ...
        if epoch % cfg.test_interval_epoch == 0:
            ...
            if test_info['activities_MPCA']>best_result['activities_MPCA']:
                best_result=test_info
            print_log(cfg.log_path, 
                      'Best group activity accuracy MPCA: %.2f%% at epoch #%d.'%(best_result['activities_MPCA'], best_result['epoch']))
            ...
            if cfg.training_stage==2:
                ...
                filepath=cfg.result_path+'/stage%d_epoch%d_MPCA_%.2f%%.pth'%(cfg.training_stage,epoch,test_info['activities_MPCA'])
                ...
                if test_info['activities_MPCA'] == best_result['activities_MPCA']:
                    for m in model.modules():
                        if isinstance(m, Basenet):
                            filepath=cfg.result_path+'/stage%d_epoch%d_MAPCA_%.2f%%.pth'%(cfg.training_stage,epoch,test_info['activities_MPCA'])
